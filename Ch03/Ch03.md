...
model should be to refactor so the single large whole is a single document.
...
the process normal government behavior/bureaucratic/Kafkaesque.^[Circle the approach choice] If I gave you a form and told you 
...
To make things interesting, we're going to use a kindergarten as our data model. As you can expect, we have the concept of children, 
...
> a child to a parent, we can use the `Include` feature to reduce the number of calls. On the other hand, `Include` wouldn't work, but we
...
In our kindergarten example, let's consider how we can model the grandparents. Each child has multiple grandparents, and each
grandparent can have multiple children. 
...
using(var session = store.OpenSession())
{
	...
	var grandparents = session.Load<Parent>(alice.Grandparents);
}
```
...
The code in Listing 3.7 will load Alice's mother and all of her grandchildren. In this case, we can see a slightly more complex query, but 
...
In this case, it might make sense to create a document just for the order header (call it `orders/2834/header`). But using a transformer
will be almost as good, and we'll discuss those in [Advanced Client API](#advanced-client-api). This saves us the need to split our data. 
...
Reference data is common, and it can be anything from a list of states to tax rates to localized text to a display on the screen. The 
...
more detail in ??XYZ_XYZ??, but RavenDB has built-in versioning capability. This can be _very_ useful when you're talking about reference
...
We need to make a distinction between several types of hierarchies. The first hierarchy is quite simple, and can comfortably fit it into a single 
...
model such a relationship as many to one or many to many, at each individual level. That's the simplest method to handling
such a requirement.
...
* `accounts/1234/txs/2017-05`&mdash;the transactions in this account on April 2017
...
aggregation. We dedicated a whole chapter ([Map/Reduce](#map-reduce)) to discussing this topic, so I'll go into the details there, but the impact
...
> **ETags in RavenDB**
> 
> RavenDB makes extensive use of the notion of etags. An etag is a 64-bit number that is incremented on every operation in RavenDB. Each
> time a document is modified, it's associated with the current etag value. The etag is used for optimistic concurrency control, for
> various internal operations and for caching. 
> 
> Etags are local for each node, and aren't globally shared. (We'll learn more about etags and change vectors in 
...
is generating the ID, it _knows_ it intends to create a new document, and it sets the expected etag accordingly. So if the document
...
The first two options use the etag that the server supplied when the document was loaded. In other words, they would error if the 
...
The ability to specify the etag on a specific entity is quite important because it allows you to perform offline optimistic concurrency
checks. You can render a web page to the user and, along with the document data there, include the etag that you got when you loaded that 
...
our transaction. That means an operation such as "the leased is signed" can be done in a single transaction that includes both the update
...
Tracking changes in data over time is a challenging task. Depending on the domain in question,^[This is common in legal and 
healthcare, for example.] we need to be able to show all changes that happened to a document. RavenDB supports that with its versioning
feature.
...
to have a backup of all changes on a per-document level.^[Obviously, this does not alleviate the need to have proper backups.] 
...
We'll discuss listeners and their use in more depth in [Advanced Client API](#advanced-client-api).
...
We'll see how to use the expiration feature in [Advanced Client API](#advanced-client-api). There isn't much to say about it besides that
...
size for a document in RavenDB. We looked at concurrency control and how etags are useful for optimistic concurrency and for caching. And we examined why we should avoid caching
aggregated data in our model (`NumberOfOrders`).

We looked at handling binary data with attachments, as well as auditing and change tracking using the versioning feature, and we learned that we can expire documents natively
in RavenDB. Reference handling at indexing and query time was briefly covered (we'll cover it in depth in the [Glorious Indexing](#advanced-indexing)), as it's important
